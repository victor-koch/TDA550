	...
	public ReversiModel() {
		...
		for (int i = 0; i < this.width; i++) {
			for (int j = 0; j < this.height; j++) {
				setGameboardState(i, j, ReversiModel.blankTile);
		...
		setGameboardState(midX, midY, ReversiModel.whiteGridTile);
		this.board[midX + 1][midY + 1] = PieceColor.WHITE;
		setGameboardState(midX + 1, midY + 1, ReversiModel.whiteGridTile);
		this.board[midX + 1][midY] = PieceColor.BLACK;
		setGameboardState(midX + 1, midY, ReversiModel.blackGridTile);
		this.board[midX][midY + 1] = PieceColor.BLACK;
		setGameboardState(midX, midY + 1, ReversiModel.blackGridTile);
				...
				while (x >= 0 && x < this.width && y >= 0 && y < this.height) {
					if (this.board[x][y] == opponentColor) {
						...
						while (!(x == cursorPos.getX() && y == cursorPos.getY())) {
							...
							setGameboardState(x, y, myColor == PieceColor.BLACK ? ReversiModel.blackGridTile
									: ReversiModel.whiteGridTile);
							x -= xDelta;
							y -= yDelta;
