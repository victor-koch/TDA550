...
public class GoldModel extends GameModel {
	...
	public GoldModel() {
		...
		for (int i = 0; i < size.width; i++) {
			for (int j = 0; j < size.height; j++) {
				setGameboardState(i, j, GoldModel.BLANK_TILE);
			}
		}

		// Insert the collector in the middle of the gameboard.
		this.collectorPos = new Position(size.width / 2, size.height / 2);
		setGameboardState(this.collectorPos, GoldModel.COLLECTOR_TILE);
	...
	private void addCoin() {
		...
		setGameboardState(newCoinPos, GoldModel.COIN_TILE);
	...
	public void gameUpdate(final int lastKey) throws GameOverException {
		...
		setGameboardState(this.collectorPos, GoldModel.BLANK_TILE);
		...
		this.coins.remove(0);
		setGameboardState(oldCoinPos, GoldModel.BLANK_TILE);
