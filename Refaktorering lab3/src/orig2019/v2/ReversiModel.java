...
public class ReversiModel implements GameModel {
	...
	public ReversiModel() {
		...
		for (int i = 0; i < this.width; i++) {
			for (int j = 0; j < this.height; j++) {
				GameUtils.setGameboardState(i, j, ReversiModel.blankTile, gameboard);
		...
		GameUtils.setGameboardState(midX, midY, ReversiModel.whiteGridTile, gameboard);
		this.board[midX + 1][midY + 1] = PieceColor.WHITE;
		GameUtils.setGameboardState(midX + 1, midY + 1, ReversiModel.whiteGridTile, gameboard);
		this.board[midX + 1][midY] = PieceColor.BLACK;
		GameUtils.setGameboardState(midX + 1, midY, ReversiModel.blackGridTile, gameboard);
		this.board[midX][midY + 1] = PieceColor.BLACK;
		GameUtils.setGameboardState(midX, midY + 1, ReversiModel.blackGridTile, gameboard);
	...
	private void tryPlay() {
		if (isPositionEmpty(this.cursorPos)) {
			...
			if (isLegalMove(this.turn, this.cursorPos)) {
				...
				GameUtils.setGameboardState(this.cursorPos, t, gameboard);
				this.board[this.cursorPos.getX()][this.cursorPos.getY()] = this.turn == Turn.BLACK ? PieceColor.BLACK
						: PieceColor.WHITE;
				System.out.println("Bong! White: " + this.whiteScore + "\tBlack: " + this.blackScore);
				...
				while (x >= 0 && x < this.width && y >= 0 && y < this.height) {
					if (this.board[x][y] == opponentColor) {
						...
						while (!(x == cursorPos.getX() && y == cursorPos.getY())) {
							...
							GameUtils.setGameboardState(x, y, myColor == PieceColor.BLACK ? ReversiModel.blackGridTile
									: ReversiModel.whiteGridTile, gameboard);
							x -= xDelta;
							y -= yDelta;
	...
	private void removeCursor(final Position oldCursorPos) {
		final GameTile t = getGameboardState(this.cursorPos);
		if (t instanceof CompositeTile) {
			final CompositeTile c = (CompositeTile) t;
			// Remove the top layer, if it is the cursor.
			if (c.getTop() == ReversiModel.cursorRedTile || c.getTop() == ReversiModel.cursorWhiteTile
					|| c.getTop() == ReversiModel.cursorBlackTile) {
				GameUtils.setGameboardState(oldCursorPos, c.getBottom(), gameboard);
			}
		}
	}
	...
	private void updateCursor() {
		...
		if (isLegalMove(this.turn, this.cursorPos)) {
			if (this.turn == Turn.BLACK) {
				cursoredTile = new CompositeTile(t, ReversiModel.cursorBlackTile);
			} else {
				cursoredTile = new CompositeTile(t, ReversiModel.cursorWhiteTile);
			}
		} else {
			cursoredTile = new CompositeTile(t, ReversiModel.cursorRedTile);
		}
		GameUtils.setGameboardState(this.cursorPos, cursoredTile, gameboard);
	}
